...
class ThreatScorer:
    ...
    def update(self, tracked_objects, loiter_ids, abandoned_bags, conflict_flag, pair_scores=None, weapons_found=None):
        persons = [o for o in tracked_objects if o["class"] == config.PERSON]
        pair_scores = pair_scores or {}
        weapons_found = weapons_found or []
        ...
        for person in persons:
            ...
            if pid in loiter_ids:
                score += 3  # loitering alone should be SUSPICIOUS
            ...
            if abandoned_bags:
                score += 4  # abandoned objects are HIGH threat

            # Weapons in frame → immediate HIGH threat for all persons
            if weapons_found:
                score += 6  # weapons = instant HIGH
