        ...
        def suspicious_frame():
            try:
                ...
                if self.suspicious_pipeline is None:
                    try:
                        # Change to the project directory so model files resolve correctly
                        ...
                        logger.info("Suspicious AI pipeline initialized successfully")
                    except ImportError as ie:
                        logger.error(f"Missing dependency for suspicious AI: {ie}")
                        return jsonify({
                            "success": False, 
                            "error": f"Missing dependency: {ie}. Install with: pip install ultralytics"
                        })
                    except Exception as init_err:
                        logger.error(f"Failed to initialize suspicious pipeline: {init_err}")
                        return jsonify({
                            "success": False, 
                            "error": f"Pipeline init failed: {init_err}"
                        })
...
def main():
    ...
    import sys
    import io
    # Fix Unicode output on Windows consoles that use cp1252
    ...
    print("=" * 60)
    print("FaceGuard Pro - Advanced Face Recognition System")
    print("=" * 60)
    ...
    try:
        ...
        print("   Suspicious Activity Detection (AI)")
        print("\nAdmin Login:")
