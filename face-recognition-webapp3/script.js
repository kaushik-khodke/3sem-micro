...
class FaceGuardPro {
    ...
    async initCamera() {
        try {
            ...
            // Use a promise with timeout to ensure loadedmetadata fires
            ...
            // In suspicious mode, make sure video is visible so user sees SOMETHING
            if (this.isSuspiciousMode) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (result.success) {
                        ...
                        consecutiveFailures++;
                        console.error("Suspicious API error:", result.error);
                        
                        if (consecutiveFailures >= MAX_FAILURES) {
                            // Show live camera feed as fallback
                            video.style.opacity = '1';
                            resultImg.style.display = 'none';
                            this.showStatus('❌ Pipeline Error: ' + (result.error || 'Unknown error'), 'error');
                            
                            // Stop retrying and tell the user
                            clearInterval(this.suspicious_interval);
                            this.suspicious_interval = null;
                            this.showNotification(
                                '❌ Suspicious AI pipeline failed to start.\n\n' +
                    ...
                    if (consecutiveFailures >= MAX_FAILURES) {
                        video.style.opacity = '1';
                        resultImg.style.display = 'none';
                        this.showStatus('❌ Server connection error', 'error');
                        clearInterval(this.suspicious_interval);
                        this.suspicious_interval = null;
                        this.showNotification('❌ Cannot reach the suspicious AI server. Is the backend running?', 'error');
                    }
                }
            } catch (err) {
                consecutiveFailures++;
                ...
                if (consecutiveFailures >= MAX_FAILURES) {
                    ...
                    clearInterval(this.suspicious_interval);
                    this.suspicious_interval = null;
                    this.showNotification('❌ Suspicious AI detection failed.\n\nCheck the browser console and server logs for details.', 'error');
                }
            }
            isProcessing = false;
